function drilldownmenu(setting){
	this.sublevelarrow={src: setting.base + 'zt_drillmenu/images/arrow.gif', width:'6px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow= setting.base +  'zt_drillmenu/images/arrow.gif' //full URL to image separating breadcrumb links (if it's defined)
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			...
			$('<li class="backcontrol"><span> Back one level</span></li>') //add back LI item
			...
			$anchorlink.click(function(e){ //assign click behavior to anchor link
				thisdrill.slidemenu($mtkb(this).data('control').order)
				e.preventDefault()
			});
		}
