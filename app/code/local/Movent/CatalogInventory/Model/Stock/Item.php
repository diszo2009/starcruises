...
class Movent_CatalogInventory_Model_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item
{
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
            $message = Mage::helper('cataloginventory')->__('As of this time, we have %s %s available. Please re-enter the quantity.', $this->getStockQty(),$this->getProductName());
            $result->setHasError(true)
                ->setMessage($message)
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
                    ...
                    if ($this->getBackorders() == Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY) {
                        ...
                        $result->setMessage(
                            Mage::helper('cataloginventory')->__('As of this time, we have %s %s available. Please re-enter quantity.', $this->getStockQty(),$this->getProductName())
                        );
                    }
                }
            } else {
