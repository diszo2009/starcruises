 ...
 */
class Movent_InfinitiumEpayment_Model_Api_Request extends Varien_Object {

	protected $_response_request_fields = array();
	protected $_infinitiumepayment_config = null;
	protected $_order;
	protected $merchant_trans_id = null;

	public function setInfinitiumEpaymentConfig($star_config) {
		$this -> _infinitiumepayment_config = $star_config;
		return $this;
	}
	...
	public function getInfinitiumEpaymentCheckoutRequest() {
		$this -> _response_request_fields['PAYMENT_METHOD'] = $this -> _infinitiumepayment_config -> getPaymentmethod();
		$this -> _response_request_fields['TRANSACTIONTYPE'] = $this -> _infinitiumepayment_config -> getTransaction();
		$this -> _response_request_fields['MERCHANTID'] = $this -> _infinitiumepayment_config -> getMerchantId();
		...
		$this -> _response_request_fields['CURRENCYCODE'] = $this -> getCurrencyCode(); // To do: Add validation for allowed currency
		...
		$this -> _response_request_fields['RESPONSE_TYPE'] = '2';
    ...
    public function getInfinitiumEpaymentWindowCheckoutRequest() {
        $this -> _response_request_fields['PAYMENT_METHOD'] = $this -> _infinitiumepayment_config -> getPaymentmethod();
        $this -> _response_request_fields['TRANSACTIONTYPE'] = $this -> _infinitiumepayment_config -> getTransaction();
        $this -> _response_request_fields['MERCHANTID'] = $this -> _infinitiumepayment_config -> getMerchantId();
        $this -> _response_request_fields['MERCHANT_TRANID'] = $this -> getMerchantTransactionId();
        ...
        $this -> _response_request_fields['DESCRIPTION'] = '';
        $this -> _response_request_fields['RESPONSE_TYPE'] = '2';
        $this -> _response_request_fields['RETURN_URL'] = $this -> getReturnUrl();
        return $this -> _response_request_fields;
    }
	...
	protected function getCardType() {
		...
		$card_types = Mage::getSingleton('infinitiumepayment/config') -> getCcTypes();
		foreach ($card_types as $code => $card_info) {
			if ($code == $card_type) {
				$card_type = $card_info['starcode'];
				break;
			}
		}
	...
	protected function getMerchantTransactionId() {
		if (is_null($this -> merchant_trans_id)) {
			...
			$this -> merchant_trans_id = "OS_" . date('mdYHis') . substr($microtime, 2, 3);
			//create sequence number for transaction id must be unique
		}
	...
	protected function getSignature() {
		...
		$encrypt = $append . $this -> _infinitiumepayment_config -> getMerchantId() . $append . $this -> _infinitiumepayment_config -> getMerchantPassword() . $append . $this -> getMerchantTransactionId() . $append . $this -> getAmount() . $append . $transaction_id . $append;

		$credits = array($this -> _infinitiumepayment_config -> getInfinitiumEpaymentUrl(), $this -> _infinitiumepayment_config -> getMerchantId(), $this -> _infinitiumepayment_config -> getMerchantPassword());
