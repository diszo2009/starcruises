<?php  
class MagenThemes_MTGeroAdmin_Block_Navigation extends Mage_Catalog_Block_Navigation
{     
	protected $_column;  
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
	{
		...
		if ($level == 0) { 
			...
			if (empty($description) || empty($urlkey) || !Mage::getStoreConfig('mtgeroadmin/navigation/show_type')) {
				$columns = 4;
			} else {
				$columns = 2;
			}
		...
		if (!empty($description) && !empty($htmlChildren) && Mage::getStoreConfig('mtgeroadmin/navigation/show_type')=='category') {
			$htmlChildren .= '<li class="menu-category-description">' . $description;
			if (Mage::getStoreConfig('mtgeroadmin/navigation/show_learn_more')) {
			...
			$htmlChildren .= '</li>';
		}else if(!empty($urlkey) && !empty($htmlChildren) && Mage::getStoreConfig('mtgeroadmin/navigation/show_type')=='static'){
